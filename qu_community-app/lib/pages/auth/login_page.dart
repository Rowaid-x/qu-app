  ...
  Future<void> _login() async {
    ...
    try {
      ...
      if (_emailController.text.trim().toLowerCase() == 'admin') {
        success = await AuthService.loginAsAdmin(
        ...
        success = await AuthService.loginAsBusDriver(
          _emailController.text.trim(),
          _passwordController.text,
        );
        if (!success) {
        ...
        success = await AuthService.loginWithQUEmail(
          _emailController.text.trim(),
          _passwordController.text,
        );
        if (!success) {
          errorMessage = 'Please use a valid @qu.edu.qa email address';
        }
      }
